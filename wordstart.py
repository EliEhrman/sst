...
max_clips_to_read = 300 # 10000 # not really how many clips but how many rows from word clip file
...
def data_input():
	...
	for irow, row in enumerate(wordfiles_reader):
		...
		word_frame_start = clip_start_pad_sizes[id] + (float(row[4]) * sr / samples_per_frame)
		word_frame_end = clip_start_pad_sizes[id] + (float(row[5]) * sr / samples_per_frame)
		# if word_frame_end == word_frame_start:
		# 	word_frame_end += 1
		starts.append(word_frame_start / float(max_clip_frames))
	...
	for iclip, word in enumerate(clipwords):
		...
		for iref, aref in enumerate(reflist):
			...
			pair_starts.append(starts[iclip])
	...
	pair_starts = [pair_starts[i] for i in shuffle_stick]

	return clip_sigs, wordsigs, pair_clip_ids, pair_word_ids, pair_starts
...
def create_graph(wordsigs, clipsigs, drop_keep_rate):
	...
	Wfc2 = _variable_with_weight_decay('wfc2', shape=[chokepoint, 1],
										  stddev=0.04, wd=0.004)
	Bfc2 = tf.get_variable('bfc2', [1], initializer=tf.constant_initializer(0.0), dtype=tf.float32)
...
clip_sigs, word_sigs, pair_clip_ids, pair_word_ids, pair_starts = data_input()
...
var_starts = tf.Variable(np.asarray(pair_starts), trainable=False)
...
t_starts = tf.slice(input_=var_starts, begin=[t_batch_begin], size=[batch_size])
...
err = tf.reduce_mean((t_final - t_starts) ** 2)
train_step1 = tf.train.AdagradOptimizer(0.5).minimize(err)
